...
uint16_t Modbus_BuildReadHoldingRegs(uint8_t slave,
                                     ...
                                     uint8_t *tx_buf)
{
    return Modbus_BuildReadRegs(slave, 0x03, reg_addr, reg_num, tx_buf);
}
                                 ...
                                 uint16_t *out_regs)
{
    ...
    if(rx[0] != slave || rx[1] != func_code) return 0;
...
uint8_t Modbus_ParseReadHoldingRegsResp(const uint8_t *rx,
                                        ...
                                        uint16_t *out_regs)
{
    return Modbus_ParseReadRegsResp(rx, rx_len, slave, 0x03, reg_num, out_regs);
}
